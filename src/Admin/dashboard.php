...
$stmt = $pdo->query("SELECT COUNT(*) FROM companies WHERE status = 'approved'");
...
// Get internship statistics by province for chart (based on companies)
$stmt = $pdo->query("
...
foreach ($province_stats as $stat) {
    $province_labels[] = $stat['province'] ?? 'ไม่ระบุ';
    $province_counts[] = $stat['count'];
}
?>
